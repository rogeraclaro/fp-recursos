	...
	return (
		<div className={theme.page}>
						...
						{isAdmin && (
							<>
								...
								<button
									...
									className='relative flex items-center gap-1.5 font-mono font-bold text-sm px-4 py-2.5 border-2 border-black bg-white shadow-[4px_4px_0px_0px_#000] hover:bg-orange-400 transition-colors'
								>
						...
						{user ? (
							<div className='flex items-center gap-2'>
								<button
									...
									className='flex items-center gap-1.5 font-mono font-bold text-sm px-4 py-2.5 border-2 border-black bg-black text-white shadow-[4px_4px_0px_0px_#000] hover:bg-gray-600 transition-colors'
								>
									<User size={16} />
								...
								.map((cat) => {
									...
									return (
										<button
											...
											className={`px-3 py-1 border border-black text-xs font-bold uppercase hover:bg-black hover:text-white transition-colors flex items-center gap-2 whitespace-nowrap shadow-[2px_2px_0px_0px_#ccc] ${isEditorCat ? 'bg-blue-100 text-black' : 'bg-white text-black'}`}
										>
											{cat.name}
					...
					<div className='bg-white border-4 border-black w-full max-w-sm max-h-[80vh] overflow-y-auto flex flex-col shadow-[8px_8px_0px_0px_#fff]'>
						...
						<div className='p-4 flex flex-col gap-3'>
								...
								.map((cat) => {
									...
									return (
										<button
											key={cat.id}
											onClick={() => scrollToCategory(cat.name)}
											className={`text-left font-bold font-mono text-lg border-2 border-black p-3 hover:bg-black hover:text-white transition-all flex justify-between items-center shadow-[4px_4px_0px_0px_#ccc] ${isEditorCat ? 'bg-blue-100' : 'bg-white'}`}
										>
											{cat.name}
											<span className='bg-orange-300 text-black text-xs px-2 py-1 border border-black'>
												{count}
											</span>
										</button>
									)
							...
							{groupedBookmarks['Altres'] && groupedBookmarks['Altres'].length > 0 && (
								<button
									onClick={() => scrollToCategory('Altres')}
									className='text-left font-bold font-mono text-lg border-2 border-black p-3 hover:bg-black hover:text-white transition-all flex justify-between items-center bg-blue-100 shadow-[4px_4px_0px_0px_#ccc]'
								>
									Altres
							...
							{highlightedBookmarks.length > 0 && (
								...
								>
									<span>DESTACAT</span>
									<span className='bg-black text-orange-400 text-xs px-2 py-1 border border-black'>
			...
			<main className='max-w-[1600px] mx-auto p-6 flex flex-col gap-12 mt-4 md:mt-4 pt-16 md:pt-4'>
				...
				{/* Secció Altres — recursos sense categoria o amb categoria eliminada */}
				{!searchQuery && groupedBookmarks['Altres'] && groupedBookmarks['Altres'].length > 0 && (
					<div id='category-Altres' className='scroll-mt-48'>
						<div className='flex items-center gap-4 mb-6'>
							<h2 className='text-3xl font-black uppercase bg-blue-100 text-black px-4 py-2 inline-block border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,0.1)]'>
								Altres
							</h2>
							<span className='font-mono font-bold text-xl text-gray-500'>
								{groupedBookmarks['Altres'].length}
								...
								{[...categories]
									...
									.map((cat) => {
										...
										return (
											<li
												key={cat.id}
												className={`flex items-center gap-2 p-3 border-2 border-black ${isEditorCat ? 'bg-blue-100' : ''}`}
											>
			...
			{deletingCat && (
				<div className='fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4'>
					<div className='bg-white border-4 border-black w-full max-w-sm shadow-[8px_8px_0px_0px_#000] p-6'>
						...
						<p className='font-mono text-sm text-blue-700 bg-blue-50 border border-blue-200 px-3 py-2 mb-4'>
						...
						<div className='flex gap-3 justify-end'>
							...
							<button
								...
								className='font-mono font-bold text-sm px-4 py-2 border-2 border-black bg-red-500 text-white hover:bg-red-600 transition-colors'
							>
								Eliminar
							</button>
						</div>
			...
			{showProfileModal && (
				<div className='fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4'>
					<div className='bg-white border-4 border-black w-full max-w-md shadow-[8px_8px_0px_0px_#000]'>
						<div className='flex justify-between items-center p-4 border-b-2 border-black bg-orange-400'>
							<h2 className='font-bold text-xl font-mono uppercase'>El meu perfil</h2>
							<button
						...
						<div className='p-6 space-y-4'>
							<div>
								<label className='font-mono text-xs font-bold uppercase text-gray-500 block mb-1'>
									Nom d'usuari
								</label>
								<input
									type='text'
									...
									className='w-full border-2 border-black p-2.5 font-mono text-sm focus:outline-none focus:bg-orange-50'
								/>
							</div>
							<div>
								<label className='font-mono text-xs font-bold uppercase text-gray-500 block mb-1'>
								...
								<input
									...
									value={user?.email ?? ''}
									readOnly
									className='w-full border-2 border-black p-2.5 font-mono text-sm bg-gray-50 text-gray-400 cursor-not-allowed'
								/>
							</div>
							<div>
								<label className='font-mono text-xs font-bold uppercase text-gray-500 block mb-1'>
							...
							<div className='flex justify-end gap-3 pt-2'>
								<button
									onClick={() => setShowProfileModal(false)}
									className='font-mono font-bold text-sm px-4 py-2.5 border-2 border-black bg-white hover:bg-gray-100 transition-colors'
								>
									Cancel·lar
								</button>
								<button
									...
									className='font-mono font-bold text-sm px-4 py-2.5 border-2 border-black bg-black text-white hover:bg-gray-800 disabled:opacity-50 transition-colors'
								>

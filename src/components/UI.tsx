...
export const Button: React.FC<ButtonProps> = ({
  ...
  const base = "font-skin font-bold text-sm px-6 py-3 border-skin transition-all duration-75 flex items-center justify-center gap-2 active:translate-x-[2px] active:translate-y-[2px] active:shadow-none outline-none focus:ring-2 focus:ring-black focus:ring-offset-2";

  const variants = {
    primary:   "bg-black text-white shadow-skin-sm hover:bg-gray-900",
    secondary: "bg-surface text-black shadow-skin-sm hover:bg-gray-50",
    danger:    "bg-red-600 text-white border-red-600 shadow-skin-sm hover:bg-red-700",
...
export const Card: React.FC<{ children: React.ReactNode; className?: string; title?: string }> = ({ children, className = '', title }) => (
  <div className={`bg-surface border-skin p-4 shadow-skin-card ${className}`}>
    {title && (
      <div className="border-b-2 border-black pb-2 mb-4 font-bold font-skin text-lg uppercase tracking-wider">
        {title}
      </div>
    )}
    {children}
...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement>> = (props) => (
  <div className="relative">
    <select
      className="w-full appearance-none bg-surface border-skin p-3 pr-8 font-skin focus:outline-none focus:bg-orange-50 shadow-[2px_2px_0px_0px_#ccc] focus:shadow-[2px_2px_0px_0px_#000] transition-all"
      {...props}
    />
...
export const Badge: React.FC<{ children: React.ReactNode; color?: string }> = ({ children, color = "bg-accent-hover" }) => (
